...
def upgrade() -> None:
    ...
    # This is the only corpus scan introduced by the metadata migration. It
    # runs once at deploy time, not whenever a Cloud Run instance starts.
    # LIMIT 2 is sufficient to distinguish one trusted vector space from a
    # mixed corpus without materializing every distinct contract.
    op.execute(
        """
        WITH contracts AS (
            SELECT
