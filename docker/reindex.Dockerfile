...
# Đây là image duy nhất đọc corpus .docx và ghi chỉ mục GraphRAG cục bộ:
# sync_external_graphrag.py dựng SQLite từ LEGAL_DATA_DIR rồi đẩy node/edge/chunk
# sang Neo4j và pgvector. build_graphrag.py dựng riêng phần SQLite khi cần.
...
COPY --chown=vlegal:vlegal scripts/build_graphrag.py ./scripts/build_graphrag.py
